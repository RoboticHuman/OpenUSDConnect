...
std::string ValueToString(const VtValue& v)
{
	if (v.IsEmpty())
		return "null";
	if (v.IsHolding<bool>())
		return v.UncheckedGet<bool>() ? "true" : "false";
	if (v.IsHolding<int>())
		return std::to_string(v.UncheckedGet<int>());
	if (v.IsHolding<int64_t>())
		return std::to_string(v.UncheckedGet<int64_t>());
	if (v.IsHolding<unsigned>())
		return std::to_string(v.UncheckedGet<unsigned>());
	if (v.IsHolding<double>())
		return std::to_string(v.UncheckedGet<double>());
	if (v.IsHolding<float>())
		return std::to_string(v.UncheckedGet<float>());
	if (v.IsHolding<std::string>())
		return '"' + v.UncheckedGet<std::string>() + '"';
	if (v.IsHolding<TfToken>())
		return '"' + v.UncheckedGet<TfToken>().GetString() + '"';
	if (v.IsHolding<SdfPath>())
		return '"' + v.UncheckedGet<SdfPath>().GetString() + '"';
...
public:
	...
	{
		byteSize_ +=
			r.layerId.size() + r.path.size() + r.field.size() + r.oldJson.size() + r.newJson.size();
	...
	const std::vector<Record>& items() const noexcept
	{
		return items_;
	}
	...
	const OucSdfDelegateRecord* data() const noexcept
	{
		return views_.data();
	}
...
public:
	explicit LayerDelegate(Tracker* owner)
		: owner_(owner)
	{
	}
...
private:
	...
	void _OnSetField(const SdfPath&, const TfToken&, const SdfAbstractDataConstValue&) override;
	void _OnSetFieldDictValueByKey(const SdfPath&, const TfToken&, const TfToken&,
	...
	void _OnSetTimeSample(const SdfPath&, double, const SdfAbstractDataConstValue&) override;
...
public:
	explicit Tracker(size_t maxBytes)
		: maxBytes_(maxBytes)
	{
	}
	~Tracker()
	...
	void setSuppressed(bool s) noexcept
	{
		suppressed_.store(s);
	}
	bool suppressed() const noexcept
	{
		return suppressed_.load();
	}
	...
	void enqueue(Record r)
	{
		...
		if (maxBytes_ && queuedBytes_ + recordBytes > maxBytes_)
		{
			...
			for (const auto& layerId : ordered)
				addOverflowMarker(layerId);
			return;
		}
		if (batches_.empty())
...
private:
	...
	{
		if (!overflowLayers_.insert(layerId).second)
			return;
		Record marker{layerId, "/", "_replaceLayerContent", {}, "null"};
		const size_t bytes = marker.layerId.size() + marker.path.size() + marker.field.size() +
							 marker.newJson.size();
		if (batches_.empty())
			batches_.emplace_back();
		queuedBytes_ += bytes;
...
bool LayerDelegate::captureOld(Record& r, const SdfPath& path, const TfToken& field) const
{
...
void LayerDelegate::_OnSetField(const SdfPath& path, const TfToken& field, const VtValue& newVal)
{
	if (owner_->suppressed())
		return;
	Record r{layerId(), path.GetString(), field.GetString(), {}, ValueToString(newVal)};
	if (captureOld(r, path, field))
		owner_->enqueue(std::move(r));
}

void LayerDelegate::_OnSetField(const SdfPath& path, const TfToken& field,
...
void LayerDelegate::_OnSetFieldDictValueByKey(const SdfPath& path, const TfToken& field,
											  const TfToken&, const VtValue&)
{
	enqueueField(path, field);
}

void LayerDelegate::_OnSetFieldDictValueByKey(const SdfPath& path, const TfToken& field,
											  const TfToken&, const SdfAbstractDataConstValue&)
{
	enqueueField(path, field);
}
...
void LayerDelegate::_OnCreateSpec(const SdfPath& path, SdfSpecType type, bool)
{
	if (owner_->suppressed())
		return;
	owner_->enqueue(
		{layerId(), path.GetString(), "_createSpec", {}, '"' + TfEnum::GetDisplayName(type) + '"'});
}
...
void LayerDelegate::_OnMoveSpec(const SdfPath& oldPath, const SdfPath& newPath)
{
	...
	owner_->enqueue({layerId(), oldPath.GetString(), "_deleteSpec", {}, "null", /*hasOld=*/true});
	owner_->enqueue({layerId(), newPath.GetString(), "_createSpec", {}, typeStr});
}

void LayerDelegate::_OnPushChild(const SdfPath&, const TfToken&, const TfToken&)
{
}

void LayerDelegate::_OnPushChild(const SdfPath&, const TfToken&, const SdfPath&)
{
}

void LayerDelegate::_OnPopChild(const SdfPath&, const TfToken&, const TfToken&)
{
}

void LayerDelegate::_OnPopChild(const SdfPath&, const TfToken&, const SdfPath&)
{
}
...
extern "C"
{
	...
	OucSdfDelegateTracker* ouc_sdf_delegate_tracker_create(const char* const* ids, size_t n,
														   size_t maxB)
	{
		try
		{
	...
	int ouc_sdf_delegate_tracker_set_layers(OucSdfDelegateTracker* t, const char* const* ids,
											size_t n)
	{
		if (!t)
			return -1;
	...
	int ouc_sdf_delegate_tracker_acquire(OucSdfDelegateTracker* t, OucSdfDelegateBatch* out)
	{
		if (!t || !out)
			return -1;
	...
	uint64_t ouc_sdf_delegate_tracker_coalesced_batch_count(const OucSdfDelegateTracker* t)
	{
