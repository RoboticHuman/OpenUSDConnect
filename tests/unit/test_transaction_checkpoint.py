...
def test_durable_checkpoint_excludes_uncommitted_sequence_reservations(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert commit.checkpoint == (epoch, head)
    finally:
        server.shutdown()
        server.store.close()


@pytest.mark.parametrize("fields", [{}, {"head_seq": 0, "epoch": 0}, {"head_seq": 42, "epoch": 3}])
def test_optional_checkpoint_roundtrip(fields):
    msg = {**make_transaction_result(1), **fields}
    decoded = message_to_dict(encode_message(msg))
    if fields:
        assert decoded["head_seq"] == fields["head_seq"]
        assert decoded["epoch"] == fields["epoch"]
    else:
        assert "head_seq" not in decoded
...
def test_checkpoint_requires_current_ack_and_no_pending_transactions(monkeypatch):
    ...
    def ack(txn_id, **fields):
        envelope = decode_envelope(encode_message({**make_transaction_result(txn_id), **fields}))
    ...
    sender._accept_result(ack(1, head_seq=8, epoch=2), generation)
    assert sender.acknowledged_checkpoint == ("server", 2, 8)
    sender._accept_result(ack(1, head_seq=999, epoch=9), generation + 1)
    assert sender.acknowledged_checkpoint == ("server", 2, 8)
    assert sender.send_events([event])
    assert sender.acknowledged_checkpoint is None
...
def test_hello_highwater_recovery_does_not_confirm_mirror(monkeypatch):
    ...
    try:
        ...
        sender._acknowledged_checkpoint = ("old-instance", 0, 10)
        generation = sender._session.begin_connection().generation
