...
from .types import TransactionCommit
...
class TransactionCoordinator:
    def __init__(
        ...
        *,
        ...
        commit_group: Callable[[list[TransactionRequest]], None] | None,
    ...
    def _commit_one(self, request: TransactionRequest) -> None:
        request.commit = None
        request.error = None
        try:
            request.commit = self._commit(request)
        except Exception as exc:
            request.error = exc

    def _complete(self, requests: list[TransactionRequest]) -> None:
        committed = [
            request for request in requests
            if request.error is None and request.commit is not None
            and request.commit.status == "committed"
        ]
        if committed:
            ...
            try:
                ...
                for request in committed:
                    request.commit = replace(request.commit, checkpoint=checkpoint)
