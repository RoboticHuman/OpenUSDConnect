...
void UUSDConnectSubsystem::AttachToStageActor(AUsdStageActor* Actor)
{
	...
	// Subscribe to prim changes for the emitter
	PrimChangedHandle = Actor->OnPrimChanged.AddUObject(
		this, &UUSDConnectSubsystem::OnPrimChanged);

	UE_LOG(LogUSDConnectSubsystem, Log,
		TEXT("Attached to AUsdStageActor (%s)"), *Actor->GetName());
}
...
void UUSDConnectSubsystem::DetachFromStageActor()
{
	...
	{
		Actor->OnPrimChanged.Remove(PrimChangedHandle);
	}
	PrimChangedHandle.Reset();
	CachedStageActor = nullptr;
}
...
// ---------------------------------------------------------------------------
// OnPrimChanged (emitter ← USD stage)
// ---------------------------------------------------------------------------

void UUSDConnectSubsystem::OnPrimChanged(const FString& PrimPath, bool bIsResync)
{
	// Skip if applying a received event (echo guard) or if the emitter isn't connected yet.
	if (bSuppressEmit.load() || !EmitClient || !EmitClient->IsConnected())
	{
		return;
	}
	...
	UE_LOG(LogUSDConnectSubsystem, Verbose,
		TEXT("Emitting prim change: %s (resync=%d)"), *PrimPath, bIsResync ? 1 : 0);
	EmitPrimChange(StageActor, PrimPath);
}
